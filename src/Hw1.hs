...
sumList :: [Int] -> Int
sumList xs = case xs of -- foldl (+) 0 xs
    [] -> 0
    (x:y) -> x + sumList y 
...
additivePersistence n = length (addits (n:[])) - 1 
    where addits :: [Int] -> [Int]
          addits xs | (head xs < 10) = xs
              | otherwise = addits ([sumList (digitsOfInt (head xs))] ++ xs)
